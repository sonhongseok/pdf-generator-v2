...
function Test-Post {
    ...
    # JSON을 파일로 저장 후 @파일명 방식으로 전달 (Windows 큰따옴표 제거 문제 해결)
...
# ─────────────────────────────────────────────
Write-Host ""
Write-Host "=== [3] PDF 생성 테스트 (각 항목 최대 3분 대기) ===" -ForegroundColor Yellow

Write-Host "  >> MERGED PDF - 시리얼 1개 (TEST-NEW10)..." -ForegroundColor Cyan
Test-Post "MERGED PDF 1개 -> 200 PDF" `
    '{"certificateDate":"2026-06-04","calibrationDate":"2026-06-04","expiryDate":"2027-06-04","serialNos":["TEST-NEW10"],"generateMode":"MERGED"}' `
    200 "pdf"

Write-Host "  >> 중복 발급 차단 (TEST-NEW10 재시도) -> 400..." -ForegroundColor Cyan
Test-Post "중복 발급 차단 -> 400" `
    '{"certificateDate":"2026-06-04","calibrationDate":"2026-06-04","expiryDate":"2027-06-04","serialNos":["TEST-NEW10"],"generateMode":"MERGED"}' `
    400

Write-Host "  >> INDIVIDUAL ZIP - 시리얼 3개 (TEST-Z01~Z03)..." -ForegroundColor Cyan
Test-Post "INDIVIDUAL ZIP 3개 -> 200 ZIP" `
    '{"certificateDate":"2026-06-04","calibrationDate":"2026-06-04","expiryDate":"2027-06-04","serialNos":["TEST-Z01","TEST-Z02","TEST-Z03"],"generateMode":"INDIVIDUAL"}' `
    200 "zip"

Write-Host "  >> MERGED PDF - 시리얼 3개 (TEST-M20~M22)..." -ForegroundColor Cyan
Test-Post "MERGED PDF 3개 -> 200 PDF" `
    '{"certificateDate":"2026-06-04","calibrationDate":"2026-06-04","expiryDate":"2027-06-04","serialNos":["TEST-M20","TEST-M21","TEST-M22"],"generateMode":"MERGED"}' `
    200 "pdf"
