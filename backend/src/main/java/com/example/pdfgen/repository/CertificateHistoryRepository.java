...
public interface CertificateHistoryRepository extends JpaRepository<CertificateHistory, Long> {
    ...
    /**
     * 특정 날짜(접두사)로 발급된 성적서 중 가장 마지막 번호의 이력을 조회합니다.
     * 시리얼 개수에 따른 다음 발급 번호 계산을 위해 연관된 매핑(serialMappings)도 함께 조회합니다.
     */
    ...
    /**
     * 발행일 + 교정일 + 만료일이 모두 동일한 발급 이력 목록을 조회합니다.
     * 4가지 입력값 완전 중복 여부 검사에 사용합니다.
     * serialMappings도 함께 로딩하여 시리얼 번호 비교에 사용합니다.
     */
    @EntityGraph(attributePaths = {"serialMappings"})
